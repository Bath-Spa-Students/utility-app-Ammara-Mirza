...
#storing the menu showing different food items.
DRINKS=[{
    "srlno":"A1",
    "name":"DEW",
    "price": 5.00,
...
# Define constants for currency and initialize the shopping cart
CURRENCY = "AED"
shopping_cart = []

# ... (Your existing code for vending machine items)
...
def display_shopping_cart():
    print(f"Shopping Cart ({CURRENCY}):")
    if not shopping_cart:
        ...
        for item in shopping_cart:
            table.add_row([item["name"], f"{CURRENCY} {item['price']}"])
...
def process_payment(total_amount):
    while True:
        try:
            payment_amount = float(input(f"Enter payment amount ({CURRENCY}): "))
            if payment_amount >= total_amount:
                change = payment_amount - total_amount
                print(f"Payment successful! Change: {CURRENCY} {change:.2f}")
                return
            else:
